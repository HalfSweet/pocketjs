...
import { BLACKBERRY_ANDROID_DEV_TARGET_ID } from "./blackberry-classic-profile.ts";
import {
  buildGuestBundle,
 ...
 */
function packageApk(resources: string, assets: string): {
  ...
  mustRun(join(buildTools, "aapt2"), [
    ...
    unsigned,
    "--manifest",
    join(appHost, "AndroidManifest.xml"),
    "-I",
    androidJar,
    "-A",
    assets,
    ...
    String(toolchain.android.apiLevel),
    "--version-code",
    String(toolchain.app.versionCode),
    "--version-name",
    toolchain.app.versionName,
    compiled,
  ]);
...
function buildApp(): void {
  ...
  const { signature, badging } = packageApk(resources, assets);
  const receipt = {
    schema: 1,
    toolchain: toolchain.toolchainVersion,
    planHash: bundle.plan.planHash,
    package: toolchain.app.package,
