...
class VersionRejectValidationError(ValueError):
    pass


@dataclass(frozen=True)
class VersionRejectSummary:
    board: str
    board_ipv4: str
    dns_answers: int
...
def fail(message: str) -> None:
    raise VersionRejectValidationError(message)
...
def integer(values: dict[str, str], name: str) -> int:
    ...
    try:
        ...
        raise VersionRejectValidationError(f"invalid integer {name}={value}") from error
...
def canonical_ipv4(value: str, option: str) -> str:
    try:
        ...
        raise VersionRejectValidationError(f"{option} must be an IPv4 literal") from error
...
def load_events(path: Path) -> list[dict[str, Any]]:
    ...
    for line_number, line in enumerate(path.read_text().splitlines(), 1):
        try:
            ...
            raise VersionRejectValidationError(
...
def validate_board_log(
    lines: list[str], board: str, peer_ipv4: str
) -> tuple[int, int]:
    ...
    boot = fields(boots[0])
    if (
        boot.get("origin") != "https://pocketjs.test:8443"
        or boot.get("peer_ipv4") != peer_ipv4
        or boot.get("public_capability") != "0"
    ):
        fail("board boot is not the exact test-only TLS version profile")
    if board == "s3":
    ...
    if (
        passed.get("mode") != "version_reject"
        or passed.get("error_code") != "tls_alert"
        or passed.get("shutdown") != "1"
        or passed.get("poison") != "0x00000000"
    ):
        fail("board did not report the expected tls_alert version rejection")
    ...
    if board == "s3":
        ...
        if (
            result.get("mode") != "version_reject"
        ...
        if passed.get("leases") != "0/0":
            fail("P4 negative-pass record does not report balanced leases")
...
def analyze(
    *,
    ...
    guest_yields, elapsed_ms = validate_board_log(lines, board, peer_ipv4)
    ...
    if (
        ...
        or tls_ready.get("tls_min_version") != "1.3"
        or tls_ready.get("tls_max_version") != "1.3"
        ...
        fail("peer readiness is not the exact TLS 1.3-only rejection profile")
    ...
    if len(handshake_errors) != 1:
        fail("TLS evidence must contain exactly one failed handshake")
    handshake_error = handshake_errors[0]
    if (
        handshake_error.get("peer_ipv4") != board_ipv4
        or handshake_error.get("error") != "SSLError"
    ):
        fail("TLS handshake failure does not belong to the selected board")

    hellos = [event for event in tls_events if event.get("event") == "tls_client_hello"]
    if len(hellos) > 1:
        fail("TLS evidence contains more than one ClientHello")
    if hellos and (
        hellos[0].get("peer_ipv4") != board_ipv4
        or hellos[0].get("server_name") != "pocketjs.test"
    ):
    ...
    if any(event.get("event") in forbidden_events for event in tls_events):
        fail("version rejection reached an opened TLS connection or HTTP request")
    ...
    if not (
        ...
        and tls_ready_ns < handshake_ns < tls_stop_ns
        and answer_ns < handshake_ns
    ):
        fail("DNS and TLS evidence is stale, incomplete, or out of order")

    return VersionRejectSummary(
        board=board,
        ...
        error_code="tls_alert",
    )


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate the PocketJS ESP Phase 1B TLS version rejection"
    )
    ...
    except (OSError, VersionRejectValidationError) as error:
        parser.exit(1, f"phase1b version rejection failed: {error}\n")
