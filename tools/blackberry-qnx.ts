...
import { BLACKBERRY_QNX_DEV_TARGET_ID } from "./blackberry-classic-profile.ts";
import {
  buildGuestBundle,
  ...
  runCommand,
  sha256File,
} from "./blackberry-classic-toolchain.ts";
...
function buildRuntime(): void {
  ...
  copyFileSync(
    join(repository, "hosts/blackberry-qnx/bar-descriptor.xml"),
  ...
  for (const symbol of [
    " main",
    " pocket_runtime_boot",
    " pocket_runtime_frame_input_ticks",
    " ui_gl_render",
  ]) {
