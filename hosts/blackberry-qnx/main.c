...
static int trackpad_primary_down;
static TouchState touch = {-1, 0, 0, 0, 0};
...
{
  switch (symbol) {
    case KEYCODE_UP: return BTN_UP;
    case KEYCODE_RIGHT: return BTN_RIGHT;
    case KEYCODE_DOWN: return BTN_DOWN;
    case KEYCODE_LEFT: return BTN_LEFT;
    case KEYCODE_RETURN: return BTN_CIRCLE;
...
static void handle_keyboard(screen_event_t event)
{
  ...
  status_dirty = 1;
  button = button_for_key(symbol);
  if (button == 0) return;
  if ((flags & SCREEN_FLAG_KEY_DOWN) != 0) {
    held_buttons |= button;
    if ((flags & SCREEN_FLAG_KEY_REPEAT) == 0) pressed_buttons |= button;
  } else {
    held_buttons &= ~button;
  }
}
...
static void handle_touch(screen_event_t event, int type)
{
  ...
  if (type == SCREEN_EVENT_MTOUCH_TOUCH) {
    ...
    /* Moves and releases apply only to the tracked contact; a second finger
     * never injects a one-frame press through the latch. */
    touch.x = position[0];
    touch.y = position[1];
    touch.latched = 1;
    if (type == SCREEN_EVENT_MTOUCH_RELEASE) touch.down = 0;
  }
}
...
static void handle_trackpad(screen_event_t event)
{
  ...
  status_dirty = 1;
  pulse_trackpad(displacement[0], displacement[1]);

  primary = buttons != 0;
  if (primary && !trackpad_primary_down) {
    held_buttons |= BTN_CIRCLE;
    pressed_buttons |= BTN_CIRCLE;
    trackpad_click_count += 1;
  } else if (!primary && trackpad_primary_down) {
    held_buttons &= ~BTN_CIRCLE;
  }
  trackpad_primary_down = primary;
}
...
static void handle_navigator_event(bps_event_t *event)
{
  ...
  switch (code) {
    ...
    case NAVIGATOR_SYSKEY_PRESS: {
      ...
      if (handled) pressed_buttons |= BTN_CIRCLE;
...
{
  uint32_t buttons = held_buttons | pressed_buttons;
  int touch_down = touch.down || touch.latched;
  int logical_x = touch.x * POCKET_LOGICAL_WIDTH / (surface_width > 0 ? surface_width : 1);
  int logical_y = touch.y * POCKET_LOGICAL_HEIGHT / (surface_height > 0 ? surface_height : 1);
  int hit = touch_down
