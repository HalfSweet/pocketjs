...
export async function generate(check: boolean): Promise<void> {
  ...
  if (check) {
    ...
    if (drifted.length > 0) {
      throw new Error(
        `formal TLS smoke generated artifact drift: ${drifted.join(", ")}; run bun generate.ts`,
      );
    }
    return;
  }
...
if (import.meta.main) {
  ...
  console.log(options.has("--check")
    ? "PocketJS formal TLS smoke artifact: generated files are current"
    : "PocketJS formal TLS smoke artifact: generated files updated");
}
