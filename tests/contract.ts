...
//  (d) Regenerates hosts/soft/pocket_spec.h (the C input constants native
...
const specHPath = new URL("../hosts/soft/pocket_spec.h", import.meta.url).pathname;
const committedHeader = await Bun.file(specHPath).text().catch(() => null);
check(
  committedHeader !== null && committedHeader === generateC(),
  "hosts/soft/pocket_spec.h matches spec.ts",
