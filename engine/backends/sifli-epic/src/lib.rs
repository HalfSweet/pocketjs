//! Hybrid PocketJS DrawList backend for the SiFli EPIC.
//!
...
/// Quarter-turn transform supported by an optional EPIC texture-copy path.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
...
/// Scale/rotate/mirror controls passed to the host's texture-copy implementation.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
...
/// Direction of a two-stop linear gradient. `from` is placed on the side
/// named by the DrawList direction's origin and `to` on its destination side.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
...
pub struct RendererConfig {
    ...
    pub min_gradient_pixels: u32,
    pub min_blend_pixels: u32,
    pub min_copy_pixels: u32,
}
...
impl Renderer {
    ...
    /// sampling remain in global viewport coordinates while all EPIC surfaces
    /// and mask offsets are translated to strip-local coordinates.
    pub fn render_strip<O: EpicOps>(
        &mut self,
        ui: &Ui,
        words: &[u32],
        destination: &mut [u16],
        ...
        if local.area() >= self.config.min_fill_pixels
            && epic.fill_rgb565(destination, width, height, local, 0)
        {
            stats.epic_fills += 1;
        } else {
    ...
    fn render_damage<O: EpicOps>(
        ...
        for &region in damage.regions() {
            ...
            if physical.area() >= self.config.min_fill_pixels
                && epic.fill_rgb565(destination, width, height, physical, 0)
            {
                stats.epic_fills += 1;
            } else {
        ...
        while i < words.len() {
            match words[i] {
                ...
                spec::draw_op::GRAD_RECT if i + 6 <= words.len() => {
                    let original = logical_rect(words[i + 1], words[i + 2]);
                    let logical = original.intersect(clip);
                    let op = &words[i..i + 6];
                    if !logical.is_empty()
                        && !self.try_gradient(
                            destination,
    ...
    fn try_gradient<O: EpicOps>(
        ...
        // HAL contract exposes a sampling-origin control.
        if logical != original || original.intersect(surface) != original {
            return false;
        }
        let Some(direction) = gradient_direction(direction) else {
            return false;
        };
        ...
        let rect = local_physical_rect(logical, surface, self.config.scale);
        if rect.is_empty() || rect.area() < self.config.min_gradient_pixels {
            return false;
        }
        if epic.fill_gradient_rgb565(destination, width, height, rect, from, to, direction) {
            stats.epic_gradients += 1;
            true
        } else {
            false
        }
        ...
        if view.psm == spec::psm::PSM_5650 {
            ...
            if destination_rect.area() >= self.config.min_copy_pixels {
                ...
                if (one_to_one || view.linear) && op[8] == 0xffff_ffff {
                    ...
                    if epic.copy_psm5650_to_rgb565(
                        destination,
                        width,
                        height,
                        view.pixels,
...
#[inline]
fn gradient_direction(direction: u32) -> Option<GradientDirection> {
    match direction {
        value if value == spec::GradDir::ToTop as u32 => Some(GradientDirection::ToTop),
        value if value == spec::GradDir::ToBottom as u32 => Some(GradientDirection::ToBottom),
        value if value == spec::GradDir::ToLeft as u32 => Some(GradientDirection::ToLeft),
        value if value == spec::GradDir::ToRight as u32 => Some(GradientDirection::ToRight),
        _ => None,
    }
}
...
/// EPIC source block offsets are integral texels. Refuse fractional UV edges
...
mod tests {
    ...
    impl EpicOps for MockEpic {
        ...
        fn fill_gradient_rgb565(
            ...
            let horizontal = matches!(
                direction,
                GradientDirection::ToLeft | GradientDirection::ToRight
            );
            let reversed = matches!(
                direction,
                GradientDirection::ToTop | GradientDirection::ToLeft
            );
            let (start, end) = if reversed { (to, from) } else { (from, to) };
            let extent = (if horizontal { rect.w } else { rect.h }) as f32;
            for y in rect.y..rect.y + rect.h {
                for x in rect.x..rect.x + rect.w {
                    let position = if horizontal { x - rect.x } else { y - rect.y };
                    let color = test_lerp_color(start, end, (position as f32 + 0.5) / extent);
                    let (r, g, b, _) = channels(color);
                    destination[(y * width + x) as usize] = pack_rgb565(r, g, b);
                }
            }
            true
        }
    ...
    fn test_lerp_color(from: u32, to: u32, factor: f32) -> u32 {
        let mix = |a: u32, b: u32| {
    ...
    #[test]
    fn strip_epic_rects_and_masks_are_local_and_mask_length_tracks_each_strip() {
        let mut ui = Ui::new();
        ui.set_viewport(16.0, 10.0);
        let words = [
            spec::draw_op::RECT,
        ...
        let mut large_epic = MockEpic::default();
        renderer
            .render_strip(
                &ui,
                &words,
            ...
            .unwrap();
        assert_eq!(large_epic.last_mask_len, 16 * 4);

        let mut small = vec![0u16; 16 * 2];
        let mut small_epic = MockEpic::default();
        renderer
            .render_strip(
                &ui,
                &words,
        ...
        assert_eq!(small_epic.last_mask_len, 16 * 2);
    }
    ...
    #[test]
    fn routes_opaque_psm5650_texture_to_epic_copy() {
        let mut ui = Ui::new();
        ui.set_viewport(8.0, 8.0);
    ...
    fn clips_psm5650_source_and_destination_without_rescaling() {
        ...
        assert_eq!(epic.last_destination_rect, epic.last_source_rect);
        assert_eq!(epic.last_transform, TextureTransform::default());
    }
