...
snapshot; no Android or QNX concept crosses the boundary:

| Physical input | Portable input |
| --- | --- |
| trackpad movement | discrete d-pad focus pulses (`SCREEN_PROPERTY_DISPLACEMENT` sign on QNX; on Android, accumulated scroll-axis or trackball deltas with a 0.35 threshold — provisional, see below) |
...
`hosts/iphone2g/pocket_runtime.c` gained two entry points for these hosts:
`pocket_runtime_frame_input_ticks` carries the button mask into the frame call
(the earlier `pocket_runtime_frame_ticks` passes zero), and
`pocket_runtime_gl_reset` drops GL resources so the backend can be
re-initialized after the platform recreates the context.
...
and packages the unsigned BAR from `hosts/blackberry-qnx/bar-descriptor.xml`.
**The tool rejects a build whose ELF is not ARM, lacks the QNX dynamic loader
or one of the four libraries, or whose BAR embeds a different executable than
the one it linked.**
...
screen remain open, so the target stays private.
...
core, compiles QuickJS, `pocket_runtime.c`, and `hosts/blackberry-android/app/jni/runtime.c`
with the plan's target id, host ABI, raster density, and logical viewport, and
links `lib/armeabi-v7a/libpocketjs.so` against `libGLESv2`, `liblog`, `libdl`,
and `libm`. **The library does not link `libandroid.so`, and `--no-undefined`
turns any missing native symbol into a link failure.** `PocketActivity.java` owns only the
Android lifecycle, the APK asset reads for `app.js` and `app.pak`, and the
raw key, touch, and trackpad callbacks; the JNI layer converts them into the
portable contract under one mutex and drives one guest tick per
`onDrawFrame`.
